...
class PointCloudSelector : public rclcpp::Node
{
public:
    PointCloudSelector() : Node("mouse_select_point"), file_index_(1), selected_points_count_(0), frame_num_(0)
    {
...
private:
    ...
    void updatePointCloud()
    {
        ...
        {
            RCLCPP_ERROR(this->get_logger(), "lidar cloud %s 不存在！", next_lidar_file.c_str());
            return;
        }
        ...
        *curr_frame_pc_ = *lidar_cloud;
        ...
        if (lidar_cluster_flag_ == true)
        {
            ...
            RCLCPP_INFO(this->get_logger(), "Shift + 鼠标左键选点，按 ‘Q’结束选点");
            ...
            RCLCPP_INFO(this->get_logger(), "重新加载点的个数为：%zu", loaded_data_.size());
    ...
    void writeCSV(int frame_number, string radar_file_name, string radar_file_format_csv, std::vector<DataObject> &data)
    {
        ...
        if (!fs::exists(directory_radar))
        {
            ...
            RCLCPP_INFO(this->get_logger(), "directory_radar %s created.", directory_radar.c_str());
        }
    ...
    void loadParams()
    {
        ...
        this->declare_parameter("file_index", 0);
        this->get_parameter("file_index", file_index_);
        std::cout << "file_index_: " << file_index_ << std::endl;

        this->declare_parameter("pcd_folder", "");
        this->get_parameter("pcd_folder", pcd_folder_);
        std::cout << " pcd_folder_:" << pcd_folder_ << std::endl;

        this->declare_parameter("save_annotation_radar_flag", false);
        this->get_parameter("save_annotation_radar_flag", save_annotation_radar_flag_);
        std::cout << " save_annotation_radar_flag:" << save_annotation_radar_flag_ << std::endl;

        this->declare_parameter("lidar_cluster_flag", false);
        this->get_parameter("lidar_cluster_flag", lidar_cluster_flag_);
        std::cout << " lidar_cluster_flag_:" << lidar_cluster_flag_ << std::endl;

        this->declare_parameter("radar_track_flag", false);
        this->get_parameter("radar_track_flag", radar_track_flag_);
        std::cout << " radar_track_flag_:" << radar_track_flag_ << std::endl;

        this->declare_parameter("file_name_annotationed", "");
        this->get_parameter("file_name_annotationed", file_name_annotationed_);
...
int main(int argc, char **argv)
{
    ...
    std::cout << "PointCloudSelector start!!!" << std::endl;
